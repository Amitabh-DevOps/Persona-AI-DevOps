...
//     const colleagues = otherPersonas.map(p => p.name).join(", ");
//     context += `
// RELATIONSHIPS:
// You co-teach the Gen AI course with ${colleagues}.
// You all share a friendly, collaborative teaching style and often promote each other’s work.
// `;
//   }
...
`;
      ...
      for (const persona of activePersonas) {
        const otherActivePersonas = activePersonas.filter(p => p.id !== persona.id);
